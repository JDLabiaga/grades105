      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 bg-white/40 backdrop-blur-md p-6 rounded-xl border border-white shadow-xl h-fit">
            <h2 className="text-[10px] font-black uppercase mb-4 opacity-60 tracking-widest">Grading Input</h2>
            <div className="space-y-4">
              <input className="w-full p-3 rounded border border-black/10 bg-white outline-none font-bold placeholder:font-normal" placeholder="Student Name" value={name} onChange={(e) => setName(e.target.value)} />
              
              <div className="space-y-2">
                {Object.keys(scores).map((key) => (
                  <div key={key} className="flex items-center gap-2 bg-white/60 p-2 rounded border border-white/20">
                    <span className="text-[9px] font-black uppercase w-20">{key === 'assign' ? 'Assignment' : key === 'atten' ? 'Attendance' : key === 'lab' ? 'Laboratory' : key}</span>
                    <input type="number" placeholder="0" className="w-full bg-transparent text-right outline-none font-bold" value={scores[key].score} onChange={(e) => setScores({...scores, [key]: {...scores[key], score: e.target.value}})} />
                    <span className="font-bold text-black/20">/</span>
                    <input type="number" placeholder="100" className="w-16 bg-transparent outline-none font-bold text-gray-500" value={scores[key].over} onChange={(e) => setScores({...scores, [key]: {...scores[key], over: e.target.value}})} />
                  </div>
                ))}
              </div>
              ...
              <button onClick={addStudent} className="w-full bg-[#2d2d2d] text-white py-4 rounded font-black uppercase text-xs hover:bg-black transition-all">
                {editingId ? 'Update Student Data' : 'Save New Record'}
              </button>
              {editingId && <button onClick={resetForm} className="w-full text-[10px] font-bold uppercase text-gray-600 mt-2">Cancel Edit</button>}
            </div>
          </div>
              ...
              <table className="w-full text-center border-collapse min-w-[1000px]">
                <thead className="bg-gray-100 border-b border-black text-[10px] font-black uppercase">
                  <tr>
                    ...
                    <th className="p-4 border-r border-black bg-blue-50">Total Raw</th>
                  ...
                  {records.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className="hover:bg-gray-50 transition-colors text-sm">
                        ...
                        <td className="p-4 border-r border-black font-black text-blue-600 bg-blue-50/20">{totalRaw}</td>
                        <td className="p-4">
