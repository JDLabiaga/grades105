...
export default function Home() {
  ...
  // State initialized to empty strings for a clean UI
  ...
  const addStudent = async () => {
    ...
    // Safely converts empty strings to 0 for the database calculation
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 bg-white/40 backdrop-blur-md p-6 rounded-xl border border-white shadow-xl h-fit">
            ...
            <div className="space-y-4">
              <input 
                className="w-full p-3 rounded border border-black/10 bg-white outline-none font-bold placeholder:font-normal" 
                ...
                {Object.keys(scores).map((key) => (
                  <div key={key} className="flex items-center gap-2 bg-white/60 p-2 rounded border border-white/20">
                    ...
                    <input 
                      ...
                      onChange={(e) => setScores({...scores, [key]: {...scores[key], score: e.target.value}})} 
                    />
                    <span className="font-bold text-black/20">/</span>
                    <input 
                      type="number" 
                      placeholder="Total"
                      className="w-16 bg-transparent outline-none font-bold text-gray-500" 
                      value={scores[key].over} 
                      onChange={(e) => setScores({...scores, [key]: {...scores[key], over: e.target.value}})} 
                    />
                  </div>
                ))}
              </div>
